...
/*   Updated: 2023/05/16 18:07:40 by alambert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
listMIS::listMIS(int argc, char ** argv) : _j(0), _N(argc - 1), _subLQ(((argc % 2) == 0 ? (argc - 1) : (argc)))	{
    ...
    std::copy(argv + 1, argv + argc, std::back_inserter(_lst));
	...
	merge_insertion_sort(_lst.begin(), _lst.end(), std::less<std::string>());

	_timeE = clock();
	std::cout << "listMis took: " << std::setprecision(10) << (((double)(_timeE - _timeS) / CLOCKS_PER_SEC) / (1e-6)) << " μs." << std::endl;
...
vectorMIS::vectorMIS(int argc, char ** argv) : _j(0), _N(argc - 1), _subVQ(((argc % 2) == 0 ? (argc - 1) : (argc)))	{
	...
	_vec.reserve(argc - 1);
    std::copy(argv + 1, argv + argc, std::back_inserter(_vec));
	...
	merge_insertion_sort(_vec.begin(), _vec.end(), std::less<std::string>());

	_timeE = clock();
	std::cout << "vectortMis took: "  << std::setprecision(10) << (((double)(_timeE - _timeS) / CLOCKS_PER_SEC) / (1e-6)) << " μs." << std::endl;
...
// -- Accessors ------------------------------------------------------------- //
std::list<std::string>		&listMIS::getL(void) {
	return (_lst);
}


std::vector<std::string>	&vectorMIS::getV(void)	{
	return (_vec);
}
// ------------------------------------------------------------- Accessors -- //
	...
	for (int i = 1; i < argc; i++)
		for (int j = 0; argv[i][j] != '\0'; j++)
			if (!std::isdigit(argv[i][j]))	{
				throw std::invalid_argument("ListMIS: Invalid argument input: only digits required.");
				return ;
			}
}
...
std::ostream &operator<<(std::ostream &out, listMIS &input)	{
	...
	out << "   After:" << std::endl;
	out << "   -------" << std::endl;
	for (std::list<std::string>::iterator it = input.getL().begin(); it != input.getL().end(); it++)
		out << *it << " ";
...
std::ostream &operator<<(std::ostream &out, vectorMIS &input)	{
	...
	out << "   Before:" << std::endl;
	out << "   -------" << std::endl;
	for (std::vector<std::string>::iterator it = input.getV().begin(); it != input.getV().end(); it++)
		out << *it << " ";
