...
/*   Updated: 2023/03/31 11:46:17 by alambert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(void) : _privateName("Default"), _privateHitPoints(10), _privateEnergyPoints(10), _privateAttackDamage(0)	{
...
ClapTrap::ClapTrap(std::string name) : _privateName(name), _privateHitPoints(10), _privateEnergyPoints(10), _privateAttackDamage(0)	{
	std::cout << MAGENTA << "Claptrap constructor called for: " << YELLOW << this->getName() << std::endl;
}

ClapTrap::ClapTrap(std::string name, int HitP, int NrgP, int AttD) : _privateName(name), _privateHitPoints(HitP), _privateEnergyPoints(NrgP), _privateAttackDamage(AttD)	{
	std::cout << MAGENTA << "Claptrap constructor called for: " << YELLOW << this->getName() << std::endl;
}
...
void			ClapTrap::attack(const std::string& target)	{ 
	if (this->getEnergyPoints() > 0 && this->getHitPoints() > 0)	{
		std::cout << YELLOW << "ClapTrap " << this->getName() << " attacks " << target << " causing " << this->getAttackDamage() << " points of damage." << std::endl;
